...
class GraphicsGridworldDisplay:
    ...
    def displayQValues(self, agent, currentState = None,currentState2 = None, message = 'Agent Q-Values'):
        qValues = util.Counter()
        states = self.gridworld.getStates()
        for state in states:
            for action in self.gridworld.getPossibleActions(state):
                qValues[(state, action)] = agent.getQValue(state, action)
        drawQValues(self.gridworld, qValues, currentState,currentState2, message)
        sleep(0.05 / self.speed)
...
LOCATION2_COLOR = formatColor(1,1,1)
...
def drawQValues(gridworld, qValues, currentState = None,  currentState2 = None ,message = 'State-Action Q-Values'):
    grid = gridworld.grid
    blank()
    ...
    for x in range(grid.width):
        for y in range(grid.height):
            ...
            isExit = (str(gridType) != gridType)
            isCurrent = (currentState == state)
            isCurrent2 = (state == currentState2)
            ...
            if gridType == '#':
                ...
                drawSquareQ(x, y, q, minValue, maxValue, valStrings, actions, isCurrent,isCurrent2)
    pos = to_screen(((grid.width - 1.0) / 2.0, - 0.8))
    text( pos, TEXT_COLOR, message, "Courier", -32, "bold", "c")
...
def drawSquareQ(x, y, qVals, minVal, maxVal, valStrs, bestActions, isCurrent, isCurrent2):
    ...
    if isCurrent2:
        circle( (screen_x, screen_y), 0.1*GRID_SIZE, LOCATION2_COLOR, fillColor=LOCATION2_COLOR )
